...
# 파일명: ./Iris.csv
import pandas as pd
df = pd.read_csv('./machine/Iris.csv')
df.info()

# [단계 2] 특정 품종 추출 (데이터 필터링)
# 전체 데이터 중 Species가 'Iris-setosa'인 데이터와  'Iris-versicolor'인 데이터만 추출하세요.

target_spec = df[df['Species'].isin(['Iris-setosa' , 'Iris-versicolor'])]
print(target_spec)
...
import numpy as np
iris_data = np.column_stack((target_spec['PetalLengthCm'] , target_spec['PetalWidthCm']))
